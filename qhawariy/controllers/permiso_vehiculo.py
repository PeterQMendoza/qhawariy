...
from qhawariy.controllers.forms.permiso_form import AgregaPermisoForm, BuscaPermisoForm
...
def mostrar_cronograma(busca):
    ...
    try:
        permisos=[{'id':p.id_dv,'start':p.fecha_inicio.strftime("%Y-%m-%d %H:%M:%S"),'end':p.fecha_final.strftime("%Y-%m-%d %H:%M:%S"),'task':p.id_vehiculo} for p in busqueda]
    except:
        permisos=[{}]

    form_busca=BuscaPermisoForm()
    titulo=f"Mostrando cronograma para {busca}"
    if form_busca.validate_on_submit():
        ...
        if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
            siguiente_pagina=url_for("cronograma_vehiculos.mostrar_cronograma",busca=buscar)
        return redirect(siguiente_pagina)
    
    return render_template("/permiso/cronograma.html",form_busca=form_busca,titulo=titulo,permisos=permisos,date=fecha,vehiculos=vehiculos)

@bp.route("/agrega_permiso",methods=["GET","POST"])
@login_required
@operacion_required
def agregar_permiso():
    ...
    form_agrega.id_vehiculo.choices=[(v.id_vehiculo,'Flota '+str(v.flota)) for v in vehiculos]
    if form_agrega.validate_on_submit():
        ...
        try:
            newDiponibleVehiculo=DisponibleVehiculo(fecha_inicio,fecha_final,id_ve)
        ...
        siguiente_pagina=request.args.get("next",None)
        if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
            siguiente_pagina=url_for("cronograma_vehiculos.mostrar_cronograma",busca='todos')
        return redirect(siguiente_pagina)
    return render_template("permiso/agrega_permiso.html",form_agrega=form_agrega)


#
@bp.route("/eliminar/<int:permiso_id>",methods=["POST"])
@login_required
@operacion_required
