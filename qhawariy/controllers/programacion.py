...
def agregar_programacion():
    ...
    if form.validate_on_submit():
        ...
        if fp is None:
            ...
            fecha.guardar()
            unico_fr=Programacion.obtener_por_fecha_y_ruta(fecha=fecha.id_fecha,ruta=ruta_f)
            if unico_fr is None:
            ...
            unico_fr=Programacion.obtener_por_fecha_y_ruta(fecha=fp.id_fecha,ruta=ruta_f)
            if unico_fr is None:
    ...
    if form.validate_on_submit():
        ...
        if vehiculo_unico is None:
            ...
            if tiempo_unico is None:
                ...
                siguiente_pagina=request.args.get("next",None)
                if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
                    siguiente_pagina=url_for("programacion.agregar_vehiculo_programado",programacion_id=pro.id_programacion)
                return redirect(siguiente_pagina)
            else:
                flash(message="El tiempo de salida debe ser único, ya hay un vehiculo con salida:{t}".format(t=tiempo),category='error')
        else:
            flash(message="Ya existe una programacion con la flota:{v}".format(v=Vehiculo.obtener_vehiculo_por_id(vp).flota),category='error')
    return render_template("programacion/lista_vehiculo_programado.html", form=form,programa=pro,vps=lista_vp,ruta=ruta)
...
def eliminar_vehiculo_programado(vp_id):
    ...
    if vp is not None:
        vp.eliminar()
    return redirect(url_for("programacion.agregar_vehiculo_programado",programacion_id=vp.id_programacion))
...
@operacion_required
def cargar_resumen():
    return stream_template("loading.html",path="resumen/",title="Cargando resumen",filename='actual')
...
def mostrar_resumen_semana(fecha):
    ...
    if form.validate_on_submit():
        ...
        siguiente_pagina=request.args.get("next",None)
        if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
            siguiente_pagina=url_for("programacion.mostrar_resumen_semana",fecha=fecha_busca)
        return redirect(siguiente_pagina)
    ...
    recorridos=["De:"+a.terminal.direccion+" A:"+a.terminal2.direccion for a in rutas_terminal]
    date_str=inicio_semana.strftime("%A %d de %B de %Y")+" al "+fin_semana.strftime("%A %d de %B de %Y")
