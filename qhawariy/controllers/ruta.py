...
from flask import (Blueprint,Response, flash, request,render_template,redirect,url_for,abort,send_file)
...
from qhawariy.controllers.forms.ruta_form import AgregarRutaForm, AgregarTerminalForm, EditarRutaForm,RutaTerminalForm
...
def agregar_ruta():
    ...
    form.terminal1.choices=[(t.id_terminal,str(t.direccion)) for t in terminales]
    form.terminal2.choices=[(t.id_terminal,str(t.direccion)) for t in terminales]
    form.proxima_ruta.choices=[(r.id_ruta,str(r.codigo)) for r in rutas]
    ...
    if form.validate_on_submit():
        ...
        ruta_terminal=RutaTerminal.obtener_por_terminales(id_terminal1=terminal1,id_terminal2=terminal2)
        if ruta_terminal is not None:
            flash(message="Existe una ruta con los terminales:{t1} y destino:{t2}".format(t1=terminal1,t2=terminal2),category='error')
        else:
            ...
            if terminal1 != terminal2:
                ...
                flash(message="Los terminales de una ruta deben encontrarse en diferentes ubicaciones. Los terminales ingresados son iguales 'Terminal 1:{t1}' y 'Terminal 2:{t2}'"
                      .format(t1=Terminal.obtener_terminal_por_id(terminal1).direccion,
                              t2=Terminal.obtener_terminal_por_id(terminal2).direccion),category='Advertencia')
...
def agregar_terminal():
    ...
    form.departamento.choices=[(d.id_departamento,str(d.nombre))for d in departamentos]
    form.provincia.choices=[(p.id_provincia,str(p.nombre))for p in provincias]
    form.distrito.choices=[(d.id_distrito,str(d.nombre))for d in distritos]
    ...
    return render_template("/ruta/agrega_terminal.html",form=form,terminales=terminales)
...
def actualizar_ruta(ruta_id):
    ...
    if ruta is None:
        ...
        form.terminal1.choices=[(t.id_terminal,str(t.direccion)) for t in terminales]
        form.terminal2.choices=[(t.id_terminal,str(t.direccion)) for t in terminales]
        form.proxima_ruta.choices=[(0,'Ninguno')]+[(r.id_ruta,str(r.codigo)) for r in rutas]
        ...
        if form.validate_on_submit():
            ...
            if terminal1 != terminal2:
                ...
                flash("La ruta {ruta_id} ha sido actualizado".format(ruta_id=ruta.codigo),category="Info")
                siguiente_pagina=request.args.get("next",None)
                if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
                    siguiente_pagina=url_for("ruta.lista_ruta")
                return redirect(siguiente_pagina)
    ...
    return render_template("ruta/edita_ruta.html",form=form,ruta=ruta,proxima=ruta_actual)
