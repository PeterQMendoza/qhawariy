...
from flask import (Blueprint, flash, render_template_string,url_for,redirect,render_template,stream_template,request,session,current_app,abort)
...
from qhawariy import login_manager,mail,cache
...
from qhawariy.controllers.forms.auth_form import CrearNuevoPasswordForm, RegisterForm,LoginForm,CambiaClaveForm,CambiaEmailForm,CambiarDatosForm, RestablecerPasswordForm
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            ...
            if len(us)==0:
                ...
                if rol_trabajador==None:
                    rol_trabajador=Rol("Trabajador")
                    rol_trabajador.guardar()
                rol=rol_trabajador
            user=Usuario(nombres=nombres,apellidos=apellidos, dni=dni,telefono=telefono,correo_electronico=email)
            user.establecer_clave(clave)
            user.guardar()
            usuario_rol=UsuarioRol(user.id_usuario,rol.id_rol) # se establece como trabajador
            usuario_rol.guardar()
            ...
            send_email(
                ...
                recipients=[email,],
                text_body=f"Hola {apellidos}{nombres}, Bienvenido a Sistema experto de Qhawariy",
                html_body=f"<p> Hola,<strong>{apellidos}{nombres}</strong>,Bienvenido al qh</p>")
    ...
    if form.validate_on_submit():
        ...
        if user is not None:
            ...
            if  is_check_password:
                login_user(user,remember=form.remember_me.data,duration=TIME_EXPIRE_LOGIN)
                next_page=request.args.get("next",None)
                if not next_page or urlparse(next_page).netloc!='':
                    next_page=url_for("home.index")
                return redirect(next_page)
            else:
                error=f"La constraseña que ingresaste no son los correctos"
        else:
...
def restablecer_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash("Las instrucciones para restablecer tu contrasenia fue enviado a tu correo electronico","info")
            return redirect(url_for("home.index"))
        else:
            flash("El correo electronico ingresado no se encuentra registrado en este sistema","error")
...
def restablecer_nuevo_password(token,usuario_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(next_page)
    return render_template("auth/restablece_nuevo_password.html",form=form,token=tok,usuario=usuario_validado)
...
def edit():
    ...
    if form_clave.validate_on_submit() and form_clave.submit_clave.data:
        if form_clave.password.data==form_clave.password_confirmado.data:
            ...
            error=f"La nueva contraseña y la contraseña de confirmacion deben ser los mismos"
    ...
    if form_datos.validate_on_submit() and form_datos.submit_datos.data:
        ...
        if not next_page or urlparse(next_page).netloc!='':
            next_page=url_for("auth.login")
        return redirect(next_page)
    return render_template("auth/edit.html", form_datos=form_datos,user=user,form_clave=form_clave,form_email=form_email, error=error)
...
<p>Hola,</p>
<p>Tu reciviste este email porque solicitaste restablecer tu password para tu cuenta.</p>
<p>
    Para restablecer tu password
    <a href="{{restablece_password_url}}">Haz click aqui</a>
</p>
<p>
    Alternativamente, tu puedes copiar el siguiente link en la barra de direccion de tu buscador:<br>
    {{restablece_password_url}}
</p>Si tu no enviaste esta solicitud para restablecer tu password, por favor contactar con alguien del equipo de desarrollo.</p>
<p>
    ¡Gracias!
</p>
"""
