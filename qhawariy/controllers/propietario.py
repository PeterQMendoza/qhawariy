...
from flask import (Blueprint, redirect,render_template, send_from_directory,stream_template,flash, url_for,request,current_app)
...
from qhawariy.controllers.forms.propietario_form import (BuscarPropietarioForm, PropietarioForm,CambiarPropietarioForm)
...
from qhawariy.utilities.files import Factory, FactoryExcel
...
def listar_propietarios():
    ...
    return stream_template("/propietario/lista_propietario.html",propietarios=propietarios,title=title,filename=NOMBRE_ARCHIVO+".xlsx",form=form)
...
def buscar_propietario(busca):
    ...
    if form.validate_on_submit():
        ...
        if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
            siguiente_pagina=url_for("propietario.buscar_propietario",busca=busca)
        return redirect(siguiente_pagina)
    return render_template("propietario/lista_propietario.html",propietarios=busqueda,title=title,filename=NOMBRE_ARCHIVO+".xlsx",form=form)
...
def agregar_propietario():
    ...
    if form.validate_on_submit():
        ...
        if propietario is not None:
            error=f"Propietario con Identidad: {documento_identificacion} ya se encuentra registrado"
        else:
        ...
        return redirect(siguiente_pagina)
    return render_template("/propietario/nuevo_propietario.html",form=form,error=error)
    ...
    if propietario is not None:
        if form.validate_on_submit():
            ...
            if not siguiente_pagina or urlparse(siguiente_pagina).netloc!='':
                siguiente_pagina=url_for("propietario.listar_propietarios")
            return redirect(siguiente_pagina)
    return render_template("propietario/edita_propietario.html",form=form,propietario=propietario)
...
def relacionar_propietario_vehiculo():
    ...
    form.propietario.choices=[(p.id_propietario,p.apellidos+", "+p.nombres) for p in propietarios]
    ...
    if form.validate_on_submit():
        ...
        return redirect(siguiente_pagina)
    return render_template("propietario/propietario_vehiculo.html",form=form,propietarios=propietarios,vehiculos=vehiculos,vps=vps,data5=da,data6=dna,pros=pros)
...
def cargar_archivo_propietarios():
    ...
    return render_template('propietario/carga_propietario.html',form=form,la=lista_archivos)
...
def descargar_archivo(name):
    propietarios=Propietario.obtener_todos_propietarios()
    excel=FactoryExcel()
    excel.crearArchivo(filename=NOMBRE_ARCHIVO,dataframe=propietarios,tipo='lista',sheetnames=['propietarios'],recorrido=None,date=None).guardar()
...
def agregar_masivo(name):
    ...
    if extension=='xlsx':
        ...
        # vehiculo=Vehiculo()
        if 'nombres' in archivo.columns and 'apellidos' in archivo.columns and 'telefono' in archivo.columns and 'documento_identificacion' in archivo.columns:
            for i in range(0,len(archivo)):
                    dni=Propietario.obtener_propietario_por_dni(archivo.iloc[i]['documento_identificacion'])
                    no=0
                    ap=0
                    te=0
                    di=0
