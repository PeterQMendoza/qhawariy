...
import folium
from folium.plugins import (Search,MarkerCluster,AntPath)
...
def mostrar_estadisticas():
    ...
    lista_rutas=[]
    for lr in rutas:
        lista_rutas.append(VehiculoProgramado.estadistica_vp_fecha_programa_y_ruta(lr.ruta.id_ruta))
    
    # Para obtener registro de 0 para cada dia si no tiene programaciones de vehiculos
    empty=copy.deepcopy(data_list1)#Copia profunda sin referencia
    for em in empty:
        for k,a in em.items():
            em.update(y=0)
    ...
    vpv=[{'x':"Flota "+str(f),'y':conteo} for vo,f,conteo in viajes_por_vehiculos]
    vpvs=[{'x':'Flota '+str(v.flota),'y':0} for v in vehiculos]#inicia con cada conteo de vehiculo 0
    # Ordenar dicccionario por valor
    ...
    lista_tvf=[{'x':f.strftime('%d-%m-%Y'),'y':c} for v,f,c in total_viajes_por_fecha]
    lista_fechas=lista_todas_las_fechas
    lista_viajes_fecha_ruta=[]
    for r in rutas:
        lista_viajes_fecha_ruta.append(Viaje.estadistica_viajes_por_fecha_ruta(r.ruta.id_ruta))

    # Inicializamos una dict con valores a 0 de todos los viajes realizados por fecha
    lvfr=copy.deepcopy(lista_tvf)#Copia profunda sin referencia
    for em in lvfr:
        for k,a in em.items():
            em.update(y=0)
...
@login_required
def mostrar_mapa():
    # obtener las coordenadas gps de las terminales, de las rutas programadas y mostrarlas
    # en el mapa
    ahora=datetime.datetime.now(tz=LIMA_TZ)
    date=datetime.datetime(year=ahora.year,month=ahora.month,day=ahora.day,hour=0,minute=0,second=0)
    ...
    tiempos_vp=[{"Flota":str(vp.vehiculo.flota),"Tiempo":vp.tiempo,"Ruta":vp.programa.ruta.codigo} for vp in vps]
    ...
    map_script_control_scale = '<script>document.addEventListener("DOMContentLoaded", function() {\
        document.querySelector("div.leaflet-bottom.leaflet-right").insertBefore(\
		    document.querySelector("div.leaflet-control-scale"), document.querySelector("div.leaflet-control-attribution")\
	    );\
    });</script>'
    ...
    map_script_control_locate = '<script>document.addEventListener("DOMContentLoaded", function() {\
        document.querySelector("div.leaflet-top.leaflet-right").insertBefore(\
		    document.querySelector("div.leaflet-control-locate"), document.querySelector("leaflet-control-layers")\
	    );\
    });</script>'
    ...
    folium.plugins.LocateControl(
        auto_start=False,
        strings={"title": "Mostrar tu posicion actual", "popup": "Tu te encuentras dentro de esta area"}
    ).add_to(m)
    ...
    # icon_seq=folium.f
    # Element('<link rel="shortcut icon" href="{{ url_for('static', filename='img/icon_seq.ico') }}" type="image/x-icon">')
