...
def buscar_vehiculo(buscar, filtro):
    ...
    title = "Resultado de buscar vehiculo: {b} por {o}".format(
        b=buscar,
        o=form.filter.choices[filtro][1]
    )
    if form.validate_on_submit():
        busca = form.busca.data
        filtro = form.filter.data
...
def descargar_archivo(name):
    ...
    excel = FactoryExcel()
    excel.crearArchivo(
        filename='lista_vehiculos',
        dataframe=vehiculos,
        tipo='lista',
        sheetnames=['vehiculos'],
        recorrido=None,
        date=None
    ).guardar()
...
def agregar_masivo(name):
    ...
    if extension == 'xlsx':
        archivo = pd.read_excel(
            current_app.config["UPLOAD_FOLDER"]+"\\"+name, index_col=0
        )
        # vehiculo=Vehiculo()
        if (
            'placa' in archivo.columns
            and 'flota' in archivo.columns
            and 'marca' in archivo.columns
            and 'modelo' in archivo.columns
            and 'fecha_fabricacion' in archivo.columns
            and 'numero_asientos' in archivo.columns
        ):
            for i in range(0, len(archivo)):
                placa = Vehiculo.obtener_vehiculo_por_placa(
                    archivo.iloc[i]['placa']
                )
                fl = 0
                pl = 0
                ma = 0
                ...
                if placa is None:
                    for a in archivo.columns:
                        r = archivo.iloc[i]
                        if a == 'flota':
                            flota = Vehiculo.obtener_vehiculo_por_flota(
                                archivo.iloc[i]['flota']
                            )
                            if flota is None:
                                fl = r[a]
                        elif a == 'placa':
