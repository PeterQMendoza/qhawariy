...
from qhawariy.models.vehiculo import (
    Vehiculo,
    EstadoViaje,
    EstadoProgramado,
    EstadoEspera
    ...
    def run(self):
        ...
        while self.is_playing:
            try:
                ...
                print(counter, aux)
                # and len(self.flota._flota)>aux:# corregir fuera de indice
                if counter > aux:
                    aux = aux+1
                else:
                    print("Termino")
...
class Flota(ControlHorario):
    ...
    def __init__(self, vehiculos: list[Vehiculo]) -> None:
        ...
        # primer vehiculo inicia su recorrido con tiempo=0
        self._horarios: list[float] = [0]
        self._flota = vehiculos
        # primer vehiculo inicia su recorrido con tiempo=0
        for i in range(len(vehiculos)-1):
