...
class RutaTerminalForm(FlaskForm):
    ...
    terminal1 = SelectField(
        "Terminal 1",
        validators=[DataRequired("Seleccione un terminal")],
        id='select_terminal_1',
        coerce=int
    )

    terminal2 = SelectField(
        "Terminal 2",
        validators=[DataRequired("Seleccione un terminal")],
        id='select_terminale_2',
        coerce=int
    )
...
class AgregarTerminalForm(FlaskForm):
    ...
    provincia = SelectField(
        "Provincia",
        validators=[DataRequired("Seleccione una provincia")],
        id='select_provincia',
        coerce=int
    )
    distrito = SelectField(
        "Distrito",
        validators=[DataRequired("Seleccione un distrito")],
        id='select_distrito',
        coerce=int
    )
...
class AgregarRutaForm(FlaskForm):
    ...
    documento = StringField(
        ...
        validators=[
            ...
            URL(
                require_tld=True,
                message="""Una URL debe contener el sufijo de "http://"+"www."+"nombre-
                    dominio"+"" ejemplos:http://www.domain.com o http://domain.com"""
            )
        ]
    )
