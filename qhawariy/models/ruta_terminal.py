...
class RutaTerminal(db.Model):
    ...
    id_terminal=db.Column(db.Integer,db.ForeignKey("terminales.id_terminal"),nullable=False)
    id_terminal_2=db.Column(db.Integer,db.ForeignKey("terminales.id_terminal"),nullable=False)

    # Relaciones
    ruta=db.relationship("Ruta",back_populates="rutas_terminal",uselist=False,single_parent=True)
    terminal=db.relationship("Terminal",foreign_keys=[id_terminal],uselist=False,single_parent=True)
    terminal2=db.relationship("Terminal",foreign_keys=[id_terminal_2],uselist=False,single_parent=True)
