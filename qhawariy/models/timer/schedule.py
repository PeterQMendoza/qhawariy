...
class ActividadBase(ABC):
    ...
    """
    # Tiempo donde inicia su programacion un vehiculo
    tiempo_salida: datetime.time
    # tiempo que tardo en completar su recorrido (desborde)
    tiempo_llegada: datetime.time
    identificador: str
    ...
    def suma_time(time1: datetime.time, time2: datetime.time) -> datetime.time:
        ...
        resultado = datetime.time(
            hour=tiempo.hour,
            minute=tiempo.minute,
            second=tiempo.second
        )
        return resultado
...
class RoundRobin:
    ...
    disponibles: List[ActividadBase] = []
    # Tiempo transcurrido
    ...
    def inicializar(self) -> None:
        ...
        for i in range(n):
            for j in range(0, n-i-1):
                a = self.lista_actividades[j].tiempo_salida
                b = self.lista_actividades[j+1].tiempo_salida
                if a > b:
        ...
        while self.lista_actividades != []:
            ...
            if self.disponibles == []:
                ...
                if rem_desborde <= self.quantum:
                    ...
                    actividad.tiempo_llegada = act.resta_time(
                        actividad.tiempo_llegada,
                        self.quantum
                    )
